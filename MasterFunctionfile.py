...
def create_manualcheck_file(input_file):
    # Load MasterV.csv
    master_df = pd.read_csv("MasterV.csv")
...
def filter_and_export(csv_file, brand_name, output_filename):
    try:
        # Read the CSV file into a DataFrame
        df = pd.read_csv(csv_file)
        
        # Filter rows where ASIN column matches the provided brand name
        filtered_df = df[df['ASIN'].str.contains("New Balance")]
